...
2D 是 1D 的张量积。对一张 $N \times N $ 图像做一次 Haar:把每个 $2\times 2 $ 小块 $\begin{pmatrix}a & b \\ c & d\end{pmatrix} $ 映射成 4 个系数:

- **LL** (低低): $(a+b+c+d)/4 $ — 局部平均(approximation)
- **LH** (低高): 水平方向上下的差 — 捕捉**水平边缘**
- **HL** (高低): 垂直方向左右的差 — 捕捉**垂直边缘**
- **HH** (高高): 对角差 — 捕捉**对角细节**

一次变换后,$N \times N $ 图像变成 4 个 $N/2 \times N/2 $ 的子图。你常见的那张"四象限图"就是这么来的:

```
┌──────┬──────┐
│  LL  │  LH  │   ← 左上是缩小的原图,右上抓水平边缘
├──────┼──────┤
│  HL  │  HH  │   ← 左下抓垂直边缘,右下抓对角细节
└──────┴──────┘
```
...
![image-20260510160825449](/Users/siyuanluo/STUDY/robot/consistency_models/literature/read/assets/image-20260510160825449.png)
